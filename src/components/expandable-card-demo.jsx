...
export default function ExpandableCardDemo() {
  ...
  return (
    <>
        ...
        {active && typeof active === "object" ? (
          <div className="fixed inset-0 grid place-items-center z-[100]">
            ...
            <motion.div
              ...
              ref={ref}
              className="w-full max-w-[500px] h-full md:h-fit md:max-h-[90%] flex flex-col bg-black border border-neutral-800 sm:rounded-3xl overflow-hidden"
            >
      ...
      <ul className="max-w-2xl mx-auto w-full gap-4 bg-black p-4 rounded-xl">
        {cards.map((card) => (
          <motion.div
            layoutId={`card-${card.title}-${id}`}
            key={`card-${card.title}-${id}`}
            onClick={() => setActive(card)}
            className="p-4 flex flex-col md:flex-row justify-between items-center hover:bg-neutral-900 rounded-xl cursor-pointer border border-neutral-800 mb-4"
          >
    ...
    content: () => {
      return (
        <p>
          ...
          performances. Directed by Mohit Suri, the film has become a significant work in the Indian film industry.{" "}
          <br /> <br /> The movie explores themes of love, redemption, and sacrifice, capturing the essence of human
          emotions and relationships. With a gripping narrative and memorable music, "Aawarapan" has garnered a massive
